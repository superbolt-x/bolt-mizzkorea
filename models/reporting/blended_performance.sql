...
paid_data as 
    (select channel, date_granularity, date, 
        ...
        0 as sho_orders, 0 as sho_quantity_ordered, 0 as sho_revenue
    from 
...
sho_data as 
    ...
    select 'Shopify' as channel, '{{date_granularity}}' as date_granularity, {{date_granularity}} as date,
        ...
        COUNT(DISTINCT order_id) as sho_orders, COALESCE(SUM(quantity),0) as sho_quantity_ordered, COALESCE(SUM(total_sales),0) as sho_revenue
...
select
    ...
    spend, impressions, clicks, add_to_cart, paid_purchases, paid_revenue, sho_orders, sho_quantity_ordered, sho_revenue
from 
